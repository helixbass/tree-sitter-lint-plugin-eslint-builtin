...
use crate::{
    get_code_path_analyzer, string_utils::upper_case_first, utils::ast_utils, CodePathOrigin,
    ON_CODE_PATH_END, ON_CODE_PATH_START,
};
...
pub fn complexity_rule() -> Arc<dyn Rule> {
    rule! {
        ...
        state => {
            ...
            [per-file-run]
            complexities: Vec<usize>,
        },
        listeners => [
            ON_CODE_PATH_START => |node, context| {
                self.complexities.push(1);
            },
            r#"
              (catch_clause) @c
            ...
            "# => |node, context| {
                *self.complexities.last_mut().unwrap() += 1;
            },
            ON_CODE_PATH_END => |node, context| {
                let code_path_analyzer = get_code_path_analyzer(context);
                let (code_path, node) = code_path_analyzer.get_on_code_path_end_payload();

                let complexity = self.complexities.pop().unwrap();
                ...
                if complexity > self.threshold {
                    let name = match code_path_analyzer.code_path_arena[code_path].origin {
...
#[cfg(test)]
mod tests {
    use crate::CodePathAnalyzerFactory;
    ...
    fn test_complexity_rule() {
        RuleTester::run_with_event_emitter(
            ...
            Arc::new(CodePathAnalyzerFactory),
        )
    }
}
