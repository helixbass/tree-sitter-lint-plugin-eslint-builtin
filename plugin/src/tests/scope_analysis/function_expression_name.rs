...
#[test]
fn test_should_create_its_special_scope() {
    tracing_subscribe();
    ...
    assert_that!(&global_scope.is_arguments_materialized()).is_true();

    let scope = &scopes[1];

    assert_that!(&scope.type_()).is_equal_to(ScopeType::FunctionExpressionName);
    let variables = scope.variables().collect_vec();
    assert_that!(&variables).has_length(1);
    assert_that!(&variables[0].name()).is_equal_to("name");
    assert_that!(&scope.is_arguments_materialized()).is_true();
    assert_that!(&scope.references().collect_vec()).is_empty();
    assert_that!(&scope.upper()).is_equal_to(global_scope);
    ...
    assert_that!(&scope.references().collect_vec()).is_empty();
    assert_that!(&scope.upper()).is_equal_to(&scopes[1]);
}
