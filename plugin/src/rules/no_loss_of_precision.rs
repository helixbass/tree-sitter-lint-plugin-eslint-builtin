...
fn get_raw(node: Node, context: &QueryMatchContext) -> String {
    node.text(context).replace("_", "")
}

fn is_base_ten(raw: &str) -> bool {
    ["0x", "0X", "0b", "0B", "0o", "0O"]
        .into_iter()
        .all(|prefix| !raw.starts_with(prefix))
        && !regex!(r#"^0[0-7]+$"#).is_match(raw)
}

fn loses_precision(node: Node, context: &QueryMatchContext) -> bool {
    let raw = get_raw(node, context);
...
pub fn no_loss_of_precision_rule() -> Arc<dyn Rule> {
    rule! {
        name => "no-loss-of-precision",
        languages => [Javascript],
        messages => [
            no_loss_of_precision => "This number literal will lose precision at runtime.",
        ],
        listeners => [
            r#"
              (number) @c
            "# => |node, context| {
                if loses_precision(node, context) {
                    context.report(violation! {
                        node => node,
                        message_id => "no_loss_of_precision",
                    });
                }
            },
        ],
...
mod tests {
    ...
    #[test]
    fn test_no_loss_of_precision_rule() {
        RuleTester::run(
            no_loss_of_precision_rule(),
            rule_tests! {
                valid => [
                    "var x = 12345",
                    "var x = 123.456",
                    ...
                    { code => "var x = 12_34_56", environment => { ecma_version => 2021 } },
                    { code => "var x = 12_3.4_56", environment => { ecma_version => 2021 } },
                    { code => "var x = -12_3.4_56", environment => { ecma_version => 2021 } },
                    { code => "var x = -12_34_56", environment => { ecma_version => 2021 } },
                    { code => "var x = 12_3e3_4", environment => { ecma_version => 2021 } },
                    { code => "var x = 123.0e3_4", environment => { ecma_version => 2021 } },
                    { code => "var x = 12_3e-3_4", environment => { ecma_version => 2021 } },
                    { code => "var x = 12_3.0e-3_4", environment => { ecma_version => 2021 } },
                    { code => "var x = -1_23e-3_4", environment => { ecma_version => 2021 } },
                    { code => "var x = -1_23.8e-3_4", environment => { ecma_version => 2021 } },
                    { code => "var x = 1_230000000_00000000_00000_000", environment => { ecma_version => 2021 } },
                    { code => "var x = -1_230000000_00000000_00000_000", environment => { ecma_version => 2021 } },
                    { code => "var x = 0.0_00_000000000_000000000_00123", environment => { ecma_version => 2021 } },
                    { code => "var x = -0.0_00_000000000_000000000_00123", environment => { ecma_version => 2021 } },
                    { code => "var x = 0e5_3", environment => { ecma_version => 2021 } },

                    { code => "var x = 0b11111111111111111111111111111111111111111111111111111", environment => { ecma_version => 6 } },
                    { code => "var x = 0b111_111_111_111_1111_11111_111_11111_1111111111_11111111_111_111", environment => { ecma_version => 2021 } },

                    { code => "var x = 0B11111111111111111111111111111111111111111111111111111", environment => { ecma_version => 6 } },
                    { code => "var x = 0B111_111_111_111_1111_11111_111_11111_1111111111_11111111_111_111", environment => { ecma_version => 2021 } },

                    { code => "var x = 0o377777777777777777", environment => { ecma_version => 6 } },
                    { code => "var x = 0o3_77_777_777_777_777_777", environment => { ecma_version => 2021 } },
                    { code => "var x = 0O377777777777777777", environment => { ecma_version => 6 } },
                    ...
                    { code => "var x = 0x1FFF_FFFF_FFF_FFF", environment => { ecma_version => 2021 } },
                    { code => "var x = 0X1_FFF_FFFF_FFF_FFF", environment => { ecma_version => 2021 } }
                ],
                invalid => [
                    {
                        code => "var x = 9007199254740993",
                        errors => [{ message_id => "no_loss_of_precision" }]
                    },
                    {
